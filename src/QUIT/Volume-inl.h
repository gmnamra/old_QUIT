...
template<typename Tp, size_t rank> VolumeBase<Tp, rank>::VolumeBase(Nifti::File &img) :
	m_storage(img.header().dims().head(rank)),
	m_xform(img.header().transform()),
...
template<typename Tp, size_t rank> void VolumeBase<Tp, rank>::readFrom(Nifti::File &img) { img.readVolumes(0, img.dim(4), m_storage.begin(), m_storage.end()); }
template<typename Tp, size_t rank> void VolumeBase<Tp, rank>::writeTo(Nifti::File &img) { img.writeVolumes(0, img.dim(4), m_storage.begin(), m_storage.end()); }
